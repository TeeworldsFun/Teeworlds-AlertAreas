...
void CAIbunny1::OnCharacterSpawn(CCharacter *pChr)
{
	...
	{
		pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetChargedWeapon(GetModularWeapon(4, 1), 2)));
		pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetStaticWeapon(SW_SHIELD)));
	...
	{
		if (frandom() < 0.5f)
			pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetModularWeapon(1, 3)));
		else
			pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetModularWeapon(3, 1)));
	...
	{
		if (frandom() < 0.5f)
			pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetModularWeapon(6, 6)));
		else
			pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetModularWeapon(6, 7)));
		
		m_AttackOnDamage = true;
		
		pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetStaticWeapon(SW_SHIELD)));
		pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetStaticWeapon(SW_SHIELD)));
		pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetStaticWeapon(SW_MASK2)));
	...
	{
		if (frandom() < 0.5f)
			pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetModularWeapon(1, 4)));
		else
			pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetModularWeapon(5, 6)));
		
		pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetStaticWeapon(SW_GRENADE2)));
		pChr->GiveCustomWeapon(GameServer()->NewWeapon(GetStaticWeapon(SW_SHIELD)));
